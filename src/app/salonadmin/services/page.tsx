...
export default async function ServicesPage() { const { salon } = await requireTenantMembership(["SALON_ADMIN"]); const [services, providers, hours] = await Promise.all([prisma.service.findMany({ where: { salonId: salon.id }, orderBy: [{ sortOrder: "asc" }, { name: "asc" }] }), prisma.provider.findMany({ where: { salonId: salon.id } }), prisma.businessHour.findMany({ where: { salonId: salon.id } })]); return <AdminShell title={salon.name} links={links}><CatalogManager tab="services" services={services} providers={providers} hours={hours} /></AdminShell>; }
