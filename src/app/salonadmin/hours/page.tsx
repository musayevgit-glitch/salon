...
export default async function HoursPage() { const { salon } = await requireTenantMembership(["SALON_ADMIN"]); const [services, providers, hours] = await Promise.all([prisma.service.findMany({ where: { salonId: salon.id } }), prisma.provider.findMany({ where: { salonId: salon.id } }), prisma.businessHour.findMany({ where: { salonId: salon.id }, orderBy: { weekday: "asc" } })]); return <AdminShell title={salon.name} links={links}><CatalogManager tab="hours" services={services} providers={providers} hours={hours} /></AdminShell>; }
